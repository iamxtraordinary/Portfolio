...
export default function AsciiCanvas({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      // Use transparent background so it can overlay smoothly
      ...
      ctx.font = `${cellH * 0.84}px var(--font-mono), monospace`;
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
          ...
          if (normMoon < 1.0) {
            ...
            opacity = clamp(0.1 + intensity * 0.7, 0.1, 0.8);
          ...
          if (!char || opacity <= 0.01) continue;

          // Amber gold tint for characters
          ctx.fillStyle = `rgba(232, 163, 23, ${opacity})`;
